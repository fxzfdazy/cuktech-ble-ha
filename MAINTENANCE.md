...
| 43 | 原版 bug 修复 | 端口开关命令读取失败时不再按 0 兜底写入，防止误关其他口 |
| 45 | 原版 bug 修复 | 修复多帧格式的 GET 响应和端口推送解码，拼接子帧后统一解密 |
...
| 44 | 新增·前端 | 开关 pending 目标值匹配解除与超时自愈 |
| 46 | 新增·协议检测 | 启发式 PD 低压段命中阈值放宽到 0.1V，覆盖带载 5.1V |
...
### 改动 43：端口开关命令读取失败误关其他口

文件：`ble_server/ble_manager.py`（`_handle_port_command`）

PIID 16 是四个口的开关位掩码，开关一个口是"读当前值 → 改一位 → 写回"。原版在 GET 16 失败（超时或响应无值）时按 0 兜底继续算，写回就只剩目标口自己的位，其余正在供电的口被全部关闭。连点几个口的开关时 BLE 链路忙，GET 更容易超时，表现为设备端乱关口（实测出现"只剩 C1 开、其余全关"）。另一个伴随问题：HTTP 层 10 秒超时后命令仍在队列里照常执行，前端已经回滚，设备却被改写，状态来回分叉。

修法：基值优先取本地 `state.settings["16"]`（每次写回后同步更新），本地缺失才向设备读；读取失败直接报错不写入，删掉 0 兜底。请求方已超时放弃（future 已取消）则跳过执行。SET 后隔 0.3 秒读回校验，值不符只告警并刷新 settings 纠正前端，不做二次盲写。

### 改动 45：修复多帧格式的 GET 响应和端口推送解码

文件：`ble_server/src/cuktech_ble/controller.py`（`_try_decode_inline`）+ `ble_server/ble_manager.py`（`_handle_multiframe`）

设备在某些 BLE session 状态下用多帧格式（`data[2]==0x00`）发送 GET 响应和端口推送，而不是内联格式（`data[2]==0x02`）。原版两处多帧处理都有 bug：controller 的 `_try_decode_inline` 收到多帧头时只 ACK 并 drain 子帧丢弃，GET/SET 响应被直接扔掉，`_recv_get_response` 等不到数据超时返回 no response；ble_manager 的 `_handle_multiframe` 把每个子帧当内联帧（strip 4 字节头）单独解密，但子帧只有 2 字节帧号前缀，偏移错位必然解密失败，连续 10 次后触发 session stale 重连死循环。

修法：两处都参照认证流程 `_recv_auth_response` 的正确做法，拼接子帧的 `frame[2:]`（strip 2 字节帧号）得到完整加密 payload 后再 `decrypt`。controller 拼接后直接解密返回明文，GET/SET 响应能被正确解码；ble_manager 拼接后构造假内联帧头（`00 00 02 00 + payload`）传给 `_try_process_inline_frame`，复用解密和端口推送处理逻辑。
...
#### 改动 44：开关 pending 目标值匹配与超时自愈

文件：`ble_server/web/static/app.js` + `phone.js`

改动 41 的 pending 只在"服务器值 === 目标值"时解除，若 SET 实际没生效、settings 轮询把服务器值改回去，pending 永远匹配不上，开关冻结在错误状态。pending 改存 `{target, at}`：服务器确认到目标值解除，超过 5 秒未确认视为过期，接受服务器状态自愈。手机版原来是请求一返回就清 pending，语义与桌面版不一致，这次一并对齐（解析响应、失败回滚、确认解除、超时自愈）。

### 协议检测

#### 改动 46：PD 低压段命中阈值放宽到 0.1V

文件：`ble_server/state_protocol_v2.py`（`_estimate_pd_subtype`）

改动 33/35 引入启发式协议推断后，C1/C2 在 PIID17 轮询窗口内（约 60 秒）走启发式判定 PD 还是 PPS。`_estimate_pd_subtype` 低压段（<12V）的 PD 命中阈值原为 0.05V，要求电压与 5/9/12V 标准档位差值 ≤0.05V 才判 PD，否则判 PPS。

实际充电头带载输出 5.1V 而非精准 5.0V（线损和电池反电势叠加的结果），距离 5V 档位 0.1V，>0.05V 阈值被误判为 PPS。重启后到 PIID17 轮询完成前的约 60 秒窗口内显示 PPS，之后切到 PD，看起来像重启后协议"随机变"。

阈值从 0.05V 放宽到 0.1V，5.1V 这类带载电压在启发式阶段也能命中 PD 档位，重启后无需等 PIID17 轮询即可正确显示 PD。和 `pdo_kind == "PD PPS"` 分支用的 0.20V 阈值仍留有差距，避免把真正的 PPS 低压档位（如 5.5V）误判成 PD。
